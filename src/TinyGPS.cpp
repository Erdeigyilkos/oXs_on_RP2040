...
float TinyGPS::distance_between (float lat1, float long1, float lat2, float long2) 
{
  ...
  float sdlong = std::sin(delta);
  float cdlong = std::cos(delta);
  lat1 = radians(lat1);
  lat2 = radians(lat2);
...
// lat/long in MILLIONTHs of a degree and age of fix in millisRp()econds
...
void TinyGPS::get_position(long *latitude, long *longitude, unsigned long *fix_age)
{
  ...
  if (fix_age) *fix_age = _last_position_fix == GPS_INVALID_FIX_TIME ? 
   GPS_INVALID_AGE : millisRp() - _last_position_fix;
}

// date as ddmmyy, time as hhmmsscc, and age in millisRp()econds
void TinyGPS::get_datetime(unsigned long *date, unsigned long *time, unsigned long *age)
{
  ...
  if (age) *age = _last_time_fix == GPS_INVALID_FIX_TIME ? 
   GPS_INVALID_AGE : millisRp() - _last_time_fix;
}
